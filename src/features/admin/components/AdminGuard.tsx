        ...
        <div className="max-w-md w-full bg-zinc-900 border border-zinc-800 rounded-3xl p-8 text-center shadow-2xl space-y-6">
          ...
          <div>
            ...
            <p className="text-sm text-zinc-400 mt-2 font-medium">
              Signed in as <span className="text-white font-bold">{currentUser.email}</span>, but this account is not registered with administrative privileges.
            </p>
          </div>
          <div className="flex gap-3">
            <button
              onClick={() => navigate('/')}
              className="flex-1 h-11 rounded-xl bg-zinc-800 hover:bg-zinc-700 text-zinc-200 text-xs font-extrabold flex items-center justify-center gap-2 transition-all cursor-pointer"
            >
